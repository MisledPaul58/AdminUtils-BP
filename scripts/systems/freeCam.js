...
var _FreeCam_instances, _FreeCam_specFreeCamGUI, _FreeCam_expFreeCamGUI, _FreeCam_expFreecamHowTo, _FreeCam_enableSpecFreeCamGUI, _FreeCam_disableSpecFreeCamGUI, _FreeCam_enableExpFreeCamGUI, _FreeCam_disableExpFreeCamGUI, _FreeCam_manageFreeCamGUI, _FreeCam_manageExpFreeCam;
import { EasingType, TicksPerSecond, system, world } from "@minecraft/server";
...
class FreeCam {
    constructor() {
        ...
        _FreeCam_manageExpFreeCam.set(this, {
            /**
             * @param { Player } p
             */
            autoChunkLoadGUI: (p) => {
                const data = database.freeCam.get(p.name);
                ...
                form.show(p).then((response) => {
                    ...
                    if (!this.isInExpFreeCam(p.name)) {
                        ...
                        switch (selection) {
                            case 0: //Back
                                __classPrivateFieldGet(this, _FreeCam_instances, "m", _FreeCam_manageFreeCamGUI).call(this, p);
                                break;
                            case 1:
                                { //Toggle state
                                    const data = database.freeCam.get(p.name);
                                    data.autoChunkLoad.enabled = !data.autoChunkLoad.enabled;
                                    database.freeCam.set(p.name, data);
                                    __classPrivateFieldGet(this, _FreeCam_manageExpFreeCam, "f").autoChunkLoadGUI(p);
                                }
                                break;
                            case 2:
                                { //Edit radius
                                    new ModalFormData()
                                        ...
                                        if (result.canceled === true)
                                            return __classPrivateFieldGet(this, _FreeCam_manageExpFreeCam, "f").autoChunkLoadGUI(p);
                                        if (!this.isInExpFreeCam(p.name)) {
                                            p.sendMessage("§cError, another user has recently disabled your freecam.");
                                            p.playSound("au.error");
                                        }
             ...
             */
            tpStartLoc: (p) => {
                const startLoc = database.freeCam.get(p.name).startLoc;
                new MessageFormData()
                    ...
                    if (result.canceled === true || result.selection === 0)
                        return __classPrivateFieldGet(this, _FreeCam_instances, "m", _FreeCam_manageFreeCamGUI).call(this, p);
                    if (!this.isInExpFreeCam(p.name)) {
                        p.sendMessage("§cError, another user has recently disabled your freecam.");
                        p.playSound("au.error");
                    }
                        ...
                        try {
                            ...
                            await new Promise(async (tp) => {
                                while (database.freeCam.get(p.name).locOverride) {
             ...
             */
            tpSpecificLoc: (p) => {
                const { lastCamLoc } = database.freeCam.get(p.name);
                new ModalFormData()
                ...
                new MessageFormData()
                    ...
                    if (!this.isInExpFreeCam(p.name)) {
                        ...
                        try {
                            p.runCommand("camera @s fade time 2 1 1 color 0 0 0");
                            await delay(40);
                            let data = database.freeCam.get(p.name);
                            data.hasToLeaveFreeCam = true;
                            database.freeCam.set(p.name, data);
                            await new Promise(async (res) => {
                                while (database.freeCam.get(p.name)) {
                                    p.runCommand("camera @s fade time 0 0.3 1 color 0 0 0");
                                    await delay(2);
                                }
                                res();
        ...
        form.button("Spectator Freecam", "textures/icons/vanish.png")
            ...
            switch (selection) {
                ...
                case 1 + extraButton: //Spectator Freecam
                    __classPrivateFieldGet(this, _FreeCam_instances, "m", _FreeCam_specFreeCamGUI).call(this, p);
                    break;
                case 2 + extraButton: //Freecam [Experimental]
                    __classPrivateFieldGet(this, _FreeCam_instances, "m", _FreeCam_expFreeCamGUI).call(this, p);
                    break;
                default:
                    break;
            }
     ...
     */
    isInFreeCam(player) {
        return database.freeCam.get(player)?.hasToLeaveFreeCam === false;
    }
    /**
     * @param { String } player
     * @returns { Boolean }
    ...
    isInSpecFreeCam(player) {
        const data = database.freeCam.get(player);
...
_FreeCam_manageExpFreeCam = new WeakMap(), _FreeCam_instances = new WeakSet(), _FreeCam_specFreeCamGUI = function _FreeCam_specFreeCamGUI(p) {
    new ActionFormData()
        ...
        switch (response.selection) {
            case 0:
                ...
                break;
            case 1:
                __classPrivateFieldGet(this, _FreeCam_instances, "m", _FreeCam_enableSpecFreeCamGUI).call(this, p);
                break;
            case 2:
                __classPrivateFieldGet(this, _FreeCam_instances, "m", _FreeCam_disableSpecFreeCamGUI).call(this, p);
                break;
            default:
                break;
        }
    });
}, _FreeCam_expFreeCamGUI = function _FreeCam_expFreeCamGUI(p) {
    new ActionFormData()
        ...
        switch (response.selection) {
            case 0:
                ...
                break;
            case 1:
                __classPrivateFieldGet(this, _FreeCam_instances, "m", _FreeCam_enableExpFreeCamGUI).call(this, p);
                break;
            case 2:
                __classPrivateFieldGet(this, _FreeCam_instances, "m", _FreeCam_disableExpFreeCamGUI).call(this, p);
                break;
            case 3:
                __classPrivateFieldGet(this, _FreeCam_instances, "m", _FreeCam_expFreecamHowTo).call(this, p);
                break;
            default:
                break;
        }
    });
}, _FreeCam_expFreecamHowTo = function _FreeCam_expFreecamHowTo(p) {
...
}, _FreeCam_enableSpecFreeCamGUI = function _FreeCam_enableSpecFreeCamGUI(p) {
    ...
    form.show(p).then((response) => {
        ...
        if (selection === 0) { //Back
            ...
            new ModalFormData()
                ...
                .show(p).then(result => {
                if (result.canceled === true)
                    return __classPrivateFieldGet(this, _FreeCam_instances, "m", _FreeCam_enableSpecFreeCamGUI).call(this, p);
                ...
                if (!isValidUsername(specifiedPlayer)) {
                    ...
                    try {
                        ...
                        if (rawPlayer) {
                            ...
                            data = {
                                mode: "spectatorFreeCam",
                                lastDimension: database.playerData.get(specifiedPlayer)?.lastDimension, //These keys with undefined values are actually lost during JSON.stringify
                                startLoc: database.playerData.get(specifiedPlayer)?.lastLoc,
                                lastGameMode: database.playerData.get(specifiedPlayer)?.lastGameMode,
                                hasToLeaveFreeCam: false
                            };
                        }
                        database.freeCam.set(specifiedPlayer, data);
                ...
                if (this.isInSpecFreeCam(selectedPlayer)) {
                    ...
                    try {
                        ...
                        if (rawPlayer) {
                            ...
                            data = {
                                mode: "spectatorFreeCam",
                                lastDimension: database.playerData.get(selectedPlayer)?.lastDimension,
                                startLoc: database.playerData.get(selectedPlayer)?.lastLoc,
                                lastGameMode: database.playerData.get(selectedPlayer)?.lastGameMode,
                                hasToLeaveFreeCam: false
                            };
                        }
                        database.freeCam.set(selectedPlayer, data);
...
}, _FreeCam_disableSpecFreeCamGUI = function _FreeCam_disableSpecFreeCamGUI(p) {
    let availablePlayers = [];
    const form = new ActionFormData()
    ...
    for (const player of database.freeCam.keys()) {
    ...
    form.show(p).then((response) => {
        ...
        if (selection === 0) { //Back
            ...
            new ModalFormData()
                ...
                .show(p).then(result => {
                if (result.canceled === true)
                    return __classPrivateFieldGet(this, _FreeCam_instances, "m", _FreeCam_disableSpecFreeCamGUI).call(this, p);
                const specifiedPlayer = result.formValues[0];
                if (!isValidUsername(specifiedPlayer)) {
                    p.sendMessage("§cError, the username you entered is invalid.");
                    p.playSound("au.error");
                    ...
                    try {
                        const rawPlayer = world.getPlayers({ name: specifiedPlayer })[0];
                        if (rawPlayer) {
                            rawPlayer.setGameMode(database.freeCam.get(specifiedPlayer).lastGameMode);
                            database.freeCam.delete(specifiedPlayer);
                        }
                        else {
                            let data = database.freeCam.get(specifiedPlayer);
                            data.hasToLeaveFreeCam = true;
                            database.freeCam.set(specifiedPlayer, data);
                        }
            ...
            new MessageFormData()
                ...
                if (this.isInExpFreeCam(selectedPlayer)) {
                    ...
                    try {
                        const rawPlayer = world.getPlayers({ name: selectedPlayer })[0];
                        if (rawPlayer) {
                            rawPlayer.setGameMode(database.freeCam.get(selectedPlayer).lastGameMode);
                            database.freeCam.delete(selectedPlayer);
                        }
                        else {
                            let data = database.freeCam.get(selectedPlayer);
                            data.hasToLeaveFreeCam = true;
                            database.freeCam.set(selectedPlayer, data);
                        }
...
}, _FreeCam_enableExpFreeCamGUI = function _FreeCam_enableExpFreeCamGUI(p) {
    let availablePlayers = [];
    const form = new ActionFormData()
        ...
        if (selection === 0) { //Back
                ...
                if (!isValidUsername(specifiedPlayer)) {
                    ...
                    try {
                        ...
                        if (rawPlayer) {
                            ...
                            data = {
                                mode: "experimentalFreeCam",
                                startLoc: database.playerData.get(specifiedPlayer)?.lastLoc, //These keys with undefined values are actually lost during JSON.stringify
                                lastCamLoc: {},
                                lastGameMode: database.playerData.get(specifiedPlayer)?.lastGameMode,
                                speed: 1.5,
                                autoChunkLoad: {
            ...
            if (this.isInSpecFreeCam(p.name)) {
                new MessageFormData()
                    ...
                    .show(p).then(result => {
                    if (result.canceled === true || result.selection === 0)
                        return __classPrivateFieldGet(this, _FreeCam_instances, "m", _FreeCam_enableExpFreeCamGUI).call(this, p);
                    if (!this.isInSpecFreeCam(p.name)) {
...
}, _FreeCam_disableExpFreeCamGUI = function _FreeCam_disableExpFreeCamGUI(p) {
    let availablePlayers = [];
    const form = new ActionFormData()
    ...
    form.show(p).then((response) => {
        ...
        if (selection === 0) { //Back
            ...
            new ModalFormData()
                ...
                .show(p).then(result => {
                if (result.canceled === true)
                    return __classPrivateFieldGet(this, _FreeCam_instances, "m", _FreeCam_disableExpFreeCamGUI).call(this, p);
                const specifiedPlayer = result.formValues[0];
                if (!isValidUsername(specifiedPlayer)) {
                    p.sendMessage("§cError, the username you entered is invalid.");
                    p.playSound("au.error");
...
}, _FreeCam_manageFreeCamGUI = function _FreeCam_manageFreeCamGUI(p) {
    if (!this.isInFreeCam(p.name)) {
        p.sendMessage("§cError, another user has recently disabled your freecam.");
        p.playSound("au.error");
    }
        ...
        if (this.isInSpecFreeCam(p.name)) {
                ...
                if (!this.isInSpecFreeCam(p.name)) {
                    ...
                    switch (selection) {
                        ...
                        case 3:
                            ...
                            break;
                        case 4:
                            { //Exit and tp to starting location
                                const { startLoc } = database.freeCam.get(p.name);
                                new MessageFormData()
                                    ...
                                    if (!this.isInSpecFreeCam(p.name)) {
                                        ...
                                        try {
                                            ...
                                            database.freeCam.delete(p.name);
                                            await delay(20);
                                            p.sendMessage("§bTeleported!");
                                            p.playSound("au.success");
                                        }
                            ...
                            { //Exit at current location
                                new MessageFormData()
                                    ...
                                    if (!this.isInSpecFreeCam(p.name)) {
                                        ...
                                        try {
                                            p.setGameMode(database.freeCam.get(p.name).lastGameMode);
                                            database.freeCam.delete(p.name);
            ...
            new ActionFormData()
                ...
                .body("Select an option")
                .button("§l<-- Back", "textures/icons/back.png") //0
                .button(`§lAuto chunk load:§r ${database.freeCam.get(p.name).autoChunkLoad.enabled ? "§aON" : "§cOFF"}§r\n§8[ §b§oClick to manage §8]`) //1
                ...
                if (!this.isInExpFreeCam(p.name)) {
                    ...
                    switch (selection) {
                        ...
                        case 2: //Tp to starting location
                            __classPrivateFieldGet(this, _FreeCam_manageExpFreeCam, "f").tpStartLoc(p);
                            break;
                        case 3: //Tp to specific location
                            __classPrivateFieldGet(this, _FreeCam_manageExpFreeCam, "f").tpSpecificLoc(p);
                            break;
                        case 4: //Tp to player in same dimension
                            __classPrivateFieldGet(this, _FreeCam_manageExpFreeCam, "f").tpToPlayerSameDim(p);
                            break;
                        case 5: //Exit and tp to starting location
                            __classPrivateFieldGet(this, _FreeCam_manageExpFreeCam, "f").startLocExit(p);
                            break;
                        case 6: //Exit at current location
                            __classPrivateFieldGet(this, _FreeCam_manageExpFreeCam, "f").tpExit(p);
                            break;
                        default:
                            break;
                    }
...
server.on("tick", () => {
    ...
    for (const player in database.freeCam.getTable()) {
        ...
        const playerData = database.freeCam.get(player);
        if (rawPlayer) {
            if (freeCam.isInSpecFreeCam(player) && playerData.lastDimension === undefined) { //If the player has joined for the first time since freecam was enabled for them
                ...
                //Save properties
                database.freeCam.set(player, newTable);
            }
...
async function handleExpFreecam(rawPlayer, startLocation, dimension) {
    ...
    let currentCamLoc = {};
    if (!database.freeCam.get(player).lastCamLoc.x) {
        Object.assign(currentCamLoc, startLoc);
        currentCamLoc.y = currentCamLoc.y + 2;
    }
    else {
        Object.assign(currentCamLoc, database.freeCam.get(player).lastCamLoc);
    }
    ...
    const run = system.runInterval(() => {
        ...
        if (data.locOverride && !data.autoChunkLoad.loading) { //Location override, used for teleporting the freecam
            let data = database.freeCam.get(player);
